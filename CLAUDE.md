...
**A DC-blocking coupling capacitor (C2) + reference resistor (R7) — decided: adopted for the `v2` PCB.** Bench testing found a large, non-audio baseline jump on cable connection alone (laptop-specific DC bias suspected; ground loops and a cable fault were ruled out). A first attempt using two polarized electrolytics wired back-to-back (a common non-polarized-cap trick) failed and stayed failed even after reseating — root cause found: that trick only tolerates brief, symmetric AC reverse-bias, not the *sustained* DC offset C2 actually needs to block, so one of the two caps was under continuous reverse bias and degraded. **C2 must be a genuinely non-polarized capacitor** (ceramic or film, ~2.2–4.7µF) — needs sourcing before Phase 3, not currently on hand. R7 (100kΩ) gives the post-cap node a defined ground reference; already tested and confirmed fine on the breadboard.
