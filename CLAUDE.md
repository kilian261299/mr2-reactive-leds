...
**Phase 2 (bench-test): in progress.** Conditioning circuit breadboarded; Stage A tuning (flash sketch, feed laptop/phone audio, watch Serial output) not yet done. Real-radio validation (originally Stage B) moved to Phase 3 — the car's RCA wiring isn't practically accessible without opening up the already-installed system. Car radio is a Kenwood DPX-07MD; no confirmed preamp voltage spec found (car head units generally ~2–4V RMS vs. a laptop's ~0.3–1V), so R3/R4 should be tuned for headroom on the bench (loud laptop content landing mid-range on the ADC, not near the ceiling) rather than precision — overshooting the ADC's 3.3V ceiling clips unrecoverably, undershooting only costs resolution that firmware can rescale later.

**Design correction found before any bench-testing:** a numeric trace through the circuit and firmware showed the originally-specified bias network (R5 10kΩ to 3.3V, R6 10kΩ to GND) set the envelope's resting voltage at 1.65V — but D1 only conducts once Node A exceeds that by its own forward-voltage drop (~0.3–0.6V), and Node A's realistic peak after the R3/R4 divider is only ~0.14–0.51V. The diode would essentially never conduct; the circuit would output a fixed ~1.65V regardless of music. **Fix: R5 removed entirely.** R6 alone now returns Node B to GND (~0V resting point instead of 1.65V), the standard single-resistor diode-envelope-detector topology — D1 only needs to clear its own forward drop, achievable with the signal actually available. Secondary effect: R6 is now the sole discharge path (previously R5 || R6 in parallel), so the decay time constant is slower than before — folded into the R3/R4/C1 retuning Stage A already covers. Documented in [docs/audio-reactive-led-plan.md](docs/audio-reactive-led-plan.md) and [hardware/audio-breakout.md](hardware/audio-breakout.md); both schematic images still show R5 and need regenerating (user is handling this).

**Phase 3 changed from the original plan.** Rather than a standalone breakout board, the audio circuit will be integrated directly into a new PCB revision (`v2`) — same EasyEDA → JLCPCB process as the original board. **PCB files are now organised by revision**: `hardware/pcb/v1/{gerbers,bom,easyeda}/` (currently installed) and `hardware/pcb/v2/` (planned, not yet designed) — mirrors how firmware versions are folder-organised. `hardware/audio-breakout.md` documents the now-superseded standalone-breakout circuit design (still accurate on the circuit itself, just not the "separate board" framing) and will need revisiting once Phase 3 starts. **Phase 3 is also where real-radio validation now happens** — on the assembled `v2` board, before permanent install; if `R3`/`R4`/`C1` need correcting from their laptop-derived Stage A values, that's a cheap hand-swap of those parts, not a re-fab.
