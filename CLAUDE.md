...
- **`J6_SWITCH` removed** — `v1`'s switch connector was never populated (just shorted instead), dropped for `v2`. New RCA input connector is **`J7`** (JST-XH 3-pin, matching the existing J2–J5 style), not yet sourced.
- **Parts still needed before starting**: non-polarized coupling capacitor (C4), BAT85 diode (D1), JST-XH 3-pin connector set (J7). Everything else needed is already on hand.

**PCB files are organised by revision**: `hardware/pcb/v1/{gerbers,bom,easyeda}/` (currently installed) and `hardware/pcb/v2/` (planned, not yet designed) — mirrors how firmware versions are folder-organised. Recommended approach: clone the `v1` EasyEDA project rather than editing it in place, so the original's design stays intact. `hardware/audio-breakout.md` documents the now-superseded standalone-breakout circuit design (still accurate on the circuit itself, just not the "separate board" framing). **Phase 3 is PCB fabrication + bench assembly (no real radio involved) plus firmware development** — visual inspection, continuity, power-on testing for the board, same as `v1`'s original validation; the new firmware mode gets written and bench-tested against Phase 2's logic in parallel, once Gerbers are submitted. **Real-radio validation happens during Phase 4's install instead**, avoiding tapping the car's RCA wiring twice (once for an early bench check, again for permanent install); if `R3`/`R4`/`C3`/`C4` need correcting at that point, that's a cheap hand-swap, not a re-fab.

**New firmware version line: `v4.x`**, branching from the current final `v2.4.1` — a new major version rather than a `v2.x` point release, to clearly distinguish the audio-reactive addition from the core motion-reactive line (`v2.x`) and the parked gyroscope experiment (`v3.x`).

**Phase 4 (install and test): not started**, depends on Phase 3. Tests, in order: the radio/speaker system still works after the RCA tap, then the original accelerometer-reactive system still works, then the new audio circuit against real radio, then the `v4.x` firmware (already developed in Phase 3) driving the real strips.
