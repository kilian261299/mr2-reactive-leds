...
Now that testing uses a spare ESP32-C3 module (see "Testing board update" above), this circuit and the testing circuit are topologically similar, down to the same `GPIO1` ADC pin — but no longer identical: besides the input source (a phone's 3.5mm jack for testing vs. the real Front L/R RCA here) and which physical ESP32-C3 module it's wired to, production also adds C4/R5 (the coupling cap and its reference resistor, never actually validated on the breadboard — see the note above) and uses D1 as a BAT85 rather than the testing circuit's 1N4007.
