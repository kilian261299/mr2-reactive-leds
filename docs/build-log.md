...
**Status:** Not started, ready to begin — Phase 2's Stage A is validated (see above). Component values and topology confirmed: R1, R2, R6, R7, C2, D1, C1 (R5, R3, and R4 all dropped). Two parts need sourcing before starting — a genuinely non-polarized capacitor for C2 (~2.2–4.7µF, ceramic or film), and a JST-XH 3-pin connector set for the new RCA input, matching the board's existing connector style. Everything else needed is already on hand.

**Decision changed from the original plan:** rather than a standalone breakout board wired to the existing PCB, the audio conditioning circuit will be added directly to a new PCB revision, alongside a new JST-XH RCA input connector matching the existing J2–J5 connector style and 2.54mm pitch. Same EasyEDA → JLCPCB process already used for the original board, including its GPIO4-fault replacement revision. See the build plan's Phase 3 for the full checklist.
