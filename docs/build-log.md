...
**Second correction, same root cause:** with R5 gone, the R3/R4 divider's original 10kΩ:1kΩ ratio (~11× attenuation) turned out to be the new bottleneck. That ratio was sized as headroom against an unconfirmed, possibly-hot car radio signal, protecting the ADC's hard 3.3V maximum — but the actual protection goal only needs ~2× attenuation, not 11×, and the excess attenuation left even a laptop's signal (~0.14V at Node A) too small to reliably clear D1's forward-drop threshold. **Fix: R3 changed 10kΩ → 1kΩ**, matching R4. This brings laptop peaks to ~0.75V at Node A, clearing D1 with margin. Documented in `docs/audio-reactive-led-plan.md` and `hardware/audio-breakout.md`.

**Car radio preout voltage confirmed, replacing the earlier estimate:** the Kenwood DPX-07MD's own service manual (`B64-2858-00`, obtained and read directly — Japanese, but the `仕様一覧`/specifications page rendered and translated) lists `プリアウトレベル (FM): 1.8V/10kΩ` — a rated preout of 1.8V RMS (~2.55V peak). This replaces the earlier unconfirmed "~2–4V RMS" guess (turns out to be lower than assumed). Getting to this number required a small side-quest — the manual's embedded Japanese font wasn't extracting as text (a CID/Adobe-Japan1 CMap resolution issue), and rendering pages as images needed `poppler` installed via `winget`, which needed its CJK data-directory path worked around before pages would render with visible text.

**Third correction, found via live bench testing rather than calculation:** breadboarding the circuit with the corrected R3=R4=1kΩ divider still in place showed no response to music at all — the combined attenuation through R2 (2.2kΩ) plus the divider left too little signal for D1 to ever conduct, matching the earlier concern about D1's margin being thin. **Fix: R3/R4 removed entirely**, and R1/R2 reduced from 2.2kΩ to 470Ω (still enough for channel isolation and short-circuit protection, without stacking on unneeded attenuation). This restored a real, visible LED response on the bench. Safe for production too: with no divider at all, ADC protection now comes purely from D1's own forward drop against the confirmed 2.55V-peak radio signal — C1 only ever charges to roughly `peak − D1's forward drop`, landing around ~1.95–2.25V at Node B, still comfortably under the ADC's 3.3V ceiling.

**A DC-blocking coupling capacitor (C2) was tried, failed, diagnosed, and ultimately decided for `v2`.** Bench testing turned up a large, non-audio baseline jump the instant the laptop's audio cable was connected, before any music played — most likely a DC bias specific to that laptop's headphone output (a ground loop and a cable fault were both tested and ruled out as the cause). A first attempt used two polarized electrolytics wired back-to-back (a common trick for approximating a non-polarized cap) plus a bleed resistor (R7) to reference the downstream node — this failed to pass any signal at all, and reseating the connections repeatedly didn't fix it. **Root cause identified:** the back-to-back trick only tolerates brief, symmetric AC reverse-bias (its intended use case), not the *sustained* DC offset C2 needs to block here — blocking a steady bias forces one of the two caps into continuous reverse bias, degrading its leakage behaviour over time, which explains a failure that survived reseating (a component problem, not a wiring one). **Decision: adopted for `v2`, but C2 must be a genuinely non-polarized part** (ceramic or film, ~2.2–4.7µF) — not yet sourced or re-tested on the bench; deferred to Phase 4 alongside the real-radio validation (Phase 3 is PCB fabrication and bench assembly only). R7 (100kΩ) was already confirmed fine and needs no change.
...
**Status:** In progress — Phase 2's Stage A is validated (see above), and every remaining circuit decision has since been finalized. This phase now also covers firmware development (moved here from what was originally planned as a Phase 4 task) — once the board's been ordered, the new firmware mode gets written and bench-tested against Phase 2's logic in parallel with manufacturing/shipping, rather than waiting idle. **New firmware version line: `v4.x`**, branching from the current final `v2.4.1` — a new major version rather than a `v2.x` point release, to clearly distinguish the audio-reactive addition from the core motion-reactive line (`v2.x`) and the parked gyroscope experiment (`v3.x`).

- **Reference designators renumbered for production.** `v1`'s real board already uses `R1`/`R2` (330Ω) and `C1`/`C2` (1000µF/100nF) for other components, so the audio circuit's parts continue straight on instead of colliding: **R3/R4** (470Ω isolation, testing-only names R1/R2), **R5** (4.7kΩ coupling reference — corrected from an original 100kΩ spec, see the Multisim finding further down this Phase 3 section; testing-only name R7), **R6** (10kΩ discharge, unchanged), **C3** (2.2µF smoothing, testing-only name C1), **C4** (coupling cap, testing-only name C2). The testing/breadboard circuit keeps its original simple R1/R2/R6/R7/C1/C2 names throughout these docs, since it's never loaded into EasyEDA and the two schemes were never meant to match 1:1.
...
- **`J6_SWITCH` removed** — `v1`'s switch connector was never populated (just shorted instead). New RCA connector is `J7` rather than reusing `J6`, so the freed-up number doesn't mean something unrelated across board revisions.
- **Parts still needed**: a genuinely non-polarized capacitor for C4 (~2.2–4.7µF, ceramic or film — the earlier back-to-back-electrolytics attempt was proven the wrong component for this job, not a wiring fault), a BAT85 diode for D1, and a JST-XH 3-pin connector set for J7. Everything else needed is already on hand.
- **The maths behind C4/R5 and the diode margin** — worked through in `docs/audio-reactive-led-plan.md` and `hardware/audio-breakout.md` — confirmed C4/R5 pass the AC signal itself with little loss (~99.5% at 20Hz with the original 100kΩ), but that analysis alone missed a separate self-biasing effect that ate into most of the signal before it reached the ADC — see the correction entry later in this Phase 3 section for the full story and the resulting R5 value change.

**Decision changed from the original plan:** rather than a standalone breakout board wired to the existing PCB, the audio conditioning circuit will be added directly to a new PCB revision, alongside the new `J7` RCA input connector matching the existing J2–J5 connector style and 2.54mm pitch. Same EasyEDA → JLCPCB process already used for the original board, including its GPIO4-fault replacement revision — recommended to clone the `v1` project rather than edit it in place. See the build plan's Phase 3 for the full checklist.
...
**Board ordered from JLCPCB.** Global Standard Direct Line shipping selected — the cheapest option, 8–12 business days quoted. Firmware development (`v4.x`) began in parallel with fabrication/shipping, per the Phase 3 plan.

**`v4.0` drafted** in `firmware/mr2-reactive-leds/v4.0/`, branching from the final `v2.4.1`. Mode 1 — previously a fixed purple theme — is replaced entirely by a bar-graph audio visualizer, ported from the validated bench-test sketch (`firmware/tests/04_audio_reactive_test/`): reads the conditioning circuit's envelope on `GPIO1`, same fast-attack/slow-release smoothing, cyan-to-orange colour snap with hysteresis (carrying over the flicker fix found on the bench sketch), drawn identically across both strips since it's one mono envelope. The bar's length represents volume; its LED brightness is a separate thing, capped by `userBrightness` using the same maths `setStrip()` already uses for every other mode — so the rotary encoder's brightness dial behaves identically in Mode 1 as anywhere else. Modes 0, 2, 3, 4 are untouched. The `Serial.availableForWrite()` fix from the bench sketch was also applied to `v2.4.1`'s pre-existing Serial calls, which never had it, since this file will be bench-tested and installed over USB.

Compiles clean (0 errors, 0 warnings) against the real `ESP32-C3 Super Mini` target, verified with `arduino-cli` rather than just eyeballed. **Not yet bench-tested against real audio** — the conditioning circuit components are with the `v2` PCB, not available for a bench rebuild. `audioFloor`/`audioCeiling` remain the Phase 2 bench placeholders (`900`/`1300`) and are expected to need retuning; the plan is to calibrate them the same way the originals were derived, from real Serial readings, during a Phase 4 semi-install directly against the car radio (ESP32 powered over USB, never simultaneously with vehicle power, so Serial stays available).

**Design error found and fixed: R5 corrected from `100kΩ` to `4.7kΩ`, after Gerbers had already been submitted.** With the physical parts unavailable for bench testing (still with the `v2` board), the conditioning circuit was instead built and tested in Multisim — driven with a real sine source and an oscilloscope on Node A/Node B, rather than trusting the existing hand calculations blind. That simulation surfaced a genuine, previously-unrecognized flaw: the original C4/R5 analysis only modelled the coupling stage as a simple AC high-pass filter (which it passes ~99.5% of signal, true as far as it goes), and never accounted for a **self-biasing effect** where D1 draws current from Node A only on the AC signal's positive peaks — since C4 blocks DC entirely, that peak current has to be exactly balanced by current flowing the other way through R5, which forces Node A's *average* voltage down well below 0V rather than centred on it. That self-bias directly eats into the peak voltage that ever reaches the diode, in a way the original transfer-function-only maths completely missed.

At the original `R5 = 100kΩ`, the resulting factor (`1/(1+R5/R6)`, with `R6 = 10kΩ`) works out to `1/11 ≈ 9%` — simulated against the confirmed real-radio peak (2.55V), Node B settled around **~0.2V**, not the ~2.2–2.4V the original "peak − Vf" shortcut had assumed throughout Phase 3. A genuinely significant gap, found only because the simulated numbers were checked against real test voltages rather than trusted on the strength of the original derivation.
...
**Status:** Not started. Depends on Phase 3. The `v4.0` firmware is drafted and compiles clean, but has not been bench-tested against real audio — Phase 3's remaining bench-testing step and Phase 4's install now effectively happen together, since the conditioning circuit parts are only available once the board is assembled. Installing the new PCB revision (replacing the currently-installed `v1` board) also happens in this phase, and is where real-radio validation happens (moved from an earlier Phase 3 plan, to avoid tapping the car's RCA wiring twice). Testing order: confirm the radio/speaker system still works after the RCA tap, then confirm the original accelerometer-reactive system still works, then validate the new audio circuit against real radio (retuning `audioFloor`/`audioCeiling` as needed), then test the `v4.0` firmware driving the real strips — see the build plan's Phase 4.
