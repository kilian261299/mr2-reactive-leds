...
**Car radio preout voltage confirmed, replacing the earlier estimate:** the Kenwood DPX-07MD's own service manual (`B64-2858-00`, obtained and read directly — Japanese, but the `仕様一覧`/specifications page rendered and translated) lists `プリアウトレベル (FM): 1.8V/10kΩ` — a rated preout of 1.8V RMS (~2.55V peak). This replaces the earlier unconfirmed "~2–4V RMS" guess (turns out to be lower than assumed) and resolves the ADC-protection question that R3/R4's `*` marking was tracking: with R3=R4=1kΩ, the real radio's worst-case Node A peak is only ~1.27V, well under the ADC's 3.3V ceiling and above D1's conduction threshold. Getting to this number required a small side-quest — the manual's embedded Japanese font wasn't extracting as text (a CID/Adobe-Japan1 CMap resolution issue), and rendering pages as images needed `poppler` installed via `winget`, which needed its CJK data-directory path worked around before pages would render with visible text. Final bench/in-car confirmation in Phase 3 remains good practice, but is no longer a safety-margin question.
