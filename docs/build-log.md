...
- None for the core project — it's finished. Active work has moved to the [audio-reactive LED addition](audio-reactive-led-plan.md); Phase 2 (bench testing) is complete and validated, Phase 3 (PCB re-fab) is in progress — `v2` has been ordered from JLCPCB and is now in fabrication/shipping. v3.0/v3.1's gyroscope approach remains parked, not being pursued further.
...
**Board ordered from JLCPCB.** Global Standard Direct Line shipping selected — the cheapest option, 8–12 business days quoted. Firmware development (`v4.x`) can now begin in parallel with fabrication/shipping, per the Phase 3 plan.

## Phase 4 — Install and Test

**Status:** Not started. Depends on Phase 3. The `v4.x` firmware is written and bench-tested in Phase 3, alongside the PCB manufacturing — this phase is where it actually gets flashed onto the installed hardware and validated for real, not where it's first written. Installing the new PCB revision (replacing the currently-installed `v1` board) also happens in this phase, and is also where real-radio validation now happens (moved from an earlier Phase 3 plan, to avoid tapping the car's RCA wiring twice). Testing order: confirm the radio/speaker system still works after the RCA tap, then confirm the original accelerometer-reactive system still works, then validate the new audio circuit against real radio, then flash and test the `v4.x` firmware driving the real strips — see the build plan's Phase 4.
