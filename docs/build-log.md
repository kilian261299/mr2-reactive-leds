...
**A DC-blocking coupling capacitor was tried and reverted.** Bench testing turned up a large, non-audio baseline jump the instant the laptop's audio cable was connected, before any music played — most likely a DC bias specific to that laptop's headphone output (a ground loop and a cable fault were both tested and ruled out as the cause). A coupling capacitor (two polarized electrolytics wired back-to-back for a non-polarized equivalent, plus a bleed resistor to reference the downstream node) would filter that out and is standard practice for audio inputs generally, but it proved fiddly to get working reliably on the breadboard, and isn't clearly needed for the real radio anyway — a proper automotive RCA preout is almost always internally AC-coupled already. Decision: **not adopted for now**, deferred as an open Phase 3 PCB-design question rather than fought over on a breadboard.
...
**Decision changed from the original plan:** rather than a standalone breakout board wired to the existing PCB, the audio conditioning circuit (R1, R2, R6, D1, C1 — R5, R3, and R4 all dropped, see the design-correction notes above) will be added directly to a new PCB revision, alongside a new RCA input connector matching the existing J2–J5 connector style. Same EasyEDA → JLCPCB process already used for the original board, including its GPIO4-fault replacement revision. See the build plan's Phase 3 for the full checklist, including the still-open decision on whether to add the coupling capacitor.
